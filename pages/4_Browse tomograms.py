...
with column_base:
    ...
    st.text("")
    ontologies = [o for o in project_configuration["ontologies"] if o != "_"] + ["Unknown"]
    for o in project_configuration["soft_ignore_in_summary"]:
        if o in ontologies:
            ontologies.remove(o)
    ontologies_values = list()
    for o in ontologies:
        ontologies_values.append(df.loc[tomo_name][o])

    ontologies, ontologies_values = zip(*sorted(zip(ontologies, ontologies_values), key=lambda x: x[1], reverse=True))
