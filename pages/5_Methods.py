...
with st.expander("Stage 1 details"):
    ...
    st.markdown("After preparing models on a desktop PC, the segmentations were computed using four Linux systems with 8 CUDA-enabled GPUs each (RTX 3080 and RTX 2080 Ti), requiring on average ~2.2 seconds per macromolecule per tomogram (note: that is by virtue of running 32 parallel processes; on a single GPU, processing a single volume costs in ~70 seconds). The tomograms binned a factor 2, to a size of 512 x 512 x 256 voxels.")
